...
// Adapted for the current app's shared-state architecture: events, chapters,
// syllabus, mocks, materials, and their setters now come from appState (the
// same source Home/Progress/Ranking read from) instead of each component
// keeping its own separate localStorage-backed copy. All logic below this
// point is untouched from the original — only where the data comes from changed.
export default function CalendarView({
  themeToggle, timerIsland, syncTrigger,
  events, setEvents, chapters, setChapters, syllabus: fullSyllabus, setSyllabus: setFullSyllabus,
  mocks, setMocks, materials,
}) {
  ...
  // Note: the original "FORCE REFRESH DATA ON MOUNT" effect and the three
  // localStorage.setItem sync effects for chapters/events/mocks are removed
  // here — appState's setEvents/setChapters/setMocks (passed in as props)
  // already persist to localStorage and trigger the Firebase sync debounce
  // internally. Keeping both would create two competing write paths to the
  // same keys. Everything else below is unchanged from the original.
  ...
  const openAddModal = (dateStr, existingTask = null) => {
    // (Original had a "force refresh from localStorage" here to work around
    // stale state. Not needed anymore — chapters/fullSyllabus are live props
    // from appState and are always current.)
  ...
  const openChapterModal = () => { 
    // (Original had a "force refresh syllabus" workaround for the
    // "Empty Syllabus" bug caused by stale local state. Not needed —
    // fullSyllabus is a live prop from appState now.)
  ...
  const toggleChapterDone = (id) => {
    setChapters(chapters.map(c => {
      if (c.id === id) {
        ...
        // (setFullSyllabus already persists to localStorage and triggers
        // the Firebase sync debounce internally — the original's direct
        // localStorage.setItem + manual syncTrigger() call here is redundant now.)
      ...
      <div className="flex-1 flex flex-col md:flex-row bg-transparent relative w-full">
        ...
        <div className="custom-calendar relative p-4 h-full flex-1 flex flex-col z-0 overflow-hidden w-full">
          <style>{`
            ...
            /* MOBILE OPTIMIZATION — replaces the original's forced 800px
               min-width (which just made phones horizontally scroll a
               full desktop-width grid). Instead, the grid itself shrinks
               to fit: smaller fonts, tighter cell padding, no forced
               scroll. This is the one intentional deviation from the
               original file per the mobile-optimization request — no
               calendar logic, data flow, or features were touched. */
            @media (max-width: 768px) {
              .custom-calendar { padding: 6px !important; }
              .custom-calendar .fc-col-header-cell { padding: 6px 0 !important; font-size: 0.62rem !important; }
              .custom-calendar .fc-daygrid-day-number { font-size: 0.72rem !important; }
              .custom-calendar .fc-toolbar-title { font-size: 1rem !important; }
              .custom-calendar .fc-daygrid-event-harness { margin-top: 2px !important; }
              .custom-calendar .fc-daygrid-day-events { padding: 0 2px !important; }
              .custom-calendar .fc-timegrid-slot-label { font-size: 0.62rem !important; }
              .custom-calendar .fc-timegrid-now-indicator-line::before { left: -30px !important; font-size: 8px !important; padding: 1px 4px !important; }
              /* Month-cell "add task" plus button and date circle sized down to fit narrow cells */
              .custom-calendar .fc-daygrid-day-top button { padding: 2px !important; }
            }
          `}</style>
